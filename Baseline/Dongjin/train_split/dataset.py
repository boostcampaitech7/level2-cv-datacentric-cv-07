...
def crop_img(img, vertices, labels, length):
    ...
    while flag and cnt < 1000:
        ...
        flag = is_cross_text([start_w, start_h], length, new_vertices[labels==1,:]) # 이미지가 크롭됐을때 바운딩 박스가 잘리는 경우는 제하기 위해서
...
class SceneTextDataset(Dataset):
    def __init__(self, root_dir,
        ...
        for nation in self._lang_list:
            with open(osp.join(json_dir, '{nation}_receipt/ufo/{json_name}'.format(nation=nation, json_name=json_name)), 'r', encoding='utf-8') as f:
                anno = json.load(f)
